...
import org.openas2.support.NetException;
...
public class AS2ReceiverHandler implements NetModuleHandler {
		...
		try {
			...
			if (msg.isRequestingMDN()) {
				...
				if (logger.isWarnEnabled())
				    logger.warn("Received message processed but MDN could not be sent for Message-ID: "
					    + msg.getMessageID());
			    }
			} else {
			    HTTPUtil.sendHTTPResponse(out, HttpURLConnection.HTTP_OK, null);
			    out.flush();
			    logger.info("Msg received, no MDN requested. Sent HTTP OK" + getClientInfo(s)
				    + msg.getLogMsgID());
			}
		    } catch (Exception e) {
	...
	try {
		...
		if (logger.isTraceEnabled()
			&& "true".equalsIgnoreCase(System.getProperty("logRxdMsgMimeBodyParts", "false"))) {
		    logger.trace("Received MimeBodyPart for inbound message after signature verification: "
			    + msg.getLogMsgID() + "\n" + MimeUtil.toString(msg.getData(), true));
		}
	    }
	} catch (Exception e) {
	...
	if (dispOptions.getMicalg() != null) {
		...
		mic = ch.calculateMIC(msg.getData(), dispOptions.getMicalg(),
			...
			msg.getPartnership().isPreventCanonicalization());
		if (logger.isDebugEnabled())
		    logger.debug("Prevent Canonicalization: " + msg.getPartnership().isPreventCanonicalization()
			    + " ::: MIC calc on rxd msg: " + mic);
		if (logger.isTraceEnabled()) {
		    ...
		    tmic = ch.calculateMIC(msg.getData(), dispOptions.getMicalg(), false,
			    msg.getPartnership().isPreventCanonicalization());
	...
	try {
		...
		if (isDecompressed) {
		    throw new DispositionException(
			    new DispositionType("automatic-action", "MDN-sent-automatically", "processed", "Error",
				    "decompression-failed"),
			    AS2ReceiverModule.DISP_DECOMPRESSION_ERROR,
     ...
     * 
     * @param msg
     *                        The received message that an MDN must be sent for.
     * @param out
     *                        The output stream for the connection the AS2 message
     *                        was received on
     * @param disposition
     *                        The disposition type that must be sent in the MDN
     * @param mic
     *                        The MIC of the received AS2 message
     * @param text
     *                        The textual message describing the result of the
    ...
    public MessageMDN createMDN(Session session, AS2Message msg, String mic, DispositionType disposition, String text)
	    throws Exception {
	...
	mdn.setAttribute(AS2MessageMDN.MDNA_REPORTING_UA,
		userAgent + "@" + msg.getAttribute(NetAttribute.MA_DESTINATION_IP) + ":"
	...
	mdn.setAttribute(AS2MessageMDN.MDNA_FINAL_RECIPIENT,
		"rfc822; " + msg.getPartnership().getReceiverID(Partnership.PID_AS2));
    ...
    public void createMDNData(Session session, MessageMDN mdn, String micAlg, String signatureProtocol)
	    throws Exception {
	...
	if (signatureProtocol != null) {
		...
		boolean isRemoveCmsAlgorithmProtectionAttr = "true"
			.equalsIgnoreCase(p.getAttribute(Partnership.PA_REMOVE_PROTECTION_ATTRIB));
		if (contentTxfrEncoding == null)
		    contentTxfrEncoding = Session.DEFAULT_CONTENT_TRANSFER_ENCODING;
		// sign the data using CryptoHelper
		MimeBodyPart signedReport = AS2Util.getCryptoHelper().sign(report, senderCert, senderKey, micAlg,
			contentTxfrEncoding, false, isRemoveCmsAlgorithmProtectionAttr);
