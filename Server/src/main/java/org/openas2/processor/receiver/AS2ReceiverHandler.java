	...
	protected String decryptAndVerify(AS2Message msg) throws OpenAS2Exception {
		...
		try {
			if (ch.isEncrypted(msg.getData())) {
				...
				// Decrypt
				if (logger.isDebugEnabled()) {
					logger.debug(msg.getLogMsgID() + " ::: decrypting");
				}
		...
		try {
			if (ch.isSigned(msg.getData())) {
				...
				if (logger.isDebugEnabled()) {
					logger.debug(msg.getLogMsgID() + " verifying signature");
				}
		...
		try {
			if (ch.isCompressed(msg.getData())) {
				if (isDecompressed) {
					throw new DispositionException(new DispositionType("automatic-action", "MDN-sent-automatically",
							"processed", "Error", "decompression-failed"), AS2ReceiverModule.DISP_DECOMPRESSION_ERROR, new Exception("Message has already been decompressed. Per RFC5402 it cannot occur twice."));
				}
				if (logger.isTraceEnabled()) {
			...
			logger.error(msg, e);
			throw new DispositionException(new DispositionType("automatic-action", "MDN-sent-automatically",
					"processed", "Error", "decompression-failed"), AS2ReceiverModule.DISP_DECOMPRESSION_ERROR, new Exception("Unexpected error occurred checking for compressed message: " + e.getMessage()));
		}
		return mic;
	}
