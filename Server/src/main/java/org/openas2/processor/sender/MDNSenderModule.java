...
public class MDNSenderModule extends HttpSenderModule {
	...
	private boolean sendAsyncMDN(MessageMDN mdn, String url, DispositionType disposition, Map<Object, Object> options)
		...
		try {
			...
			if ((respCode != HttpURLConnection.HTTP_OK) && (respCode != HttpURLConnection.HTTP_CREATED)
					&& (respCode != HttpURLConnection.HTTP_ACCEPTED) && (respCode != HttpURLConnection.HTTP_PARTIAL)
					&& (respCode != HttpURLConnection.HTTP_NO_CONTENT)) {
				if (logger.isErrorEnabled()) {
					msg.setLogMsg(
							"Error sending AsyncMDN [" + disposition.toString() + "] HTTP response code: " + respCode);
					logger.error(msg);
				}
			...
			logger.warn("HTTP exception sending ASYNC MDN: " + org.openas2.logging.Log.getExceptionMsg(hre)
					+ msg.getLogMsgID(), hre);
			hre.terminate();
			resend(msg, hre);
			// Log significant msg state
	...
	protected void resend(Message msg, OpenAS2Exception cause) throws OpenAS2Exception {
		...
		if (logger.isDebugEnabled())
			logger.debug("MDN resend retries: MSG - " + msgOptions.get(SenderModule.SOPT_RETRIES) + "   ::: RETRIES - "
					+ tries);
		int retries = -1;
		if (tries == null)
