...
public class AS2SenderModule extends HttpSenderModule implements HasSchedule {
	...
	if (logger.isTraceEnabled()) {
		...
		logger.trace("Message object in sender module. Content-Disposition: " + msg.getContentDisposition()
			...
			+ AS2Util.printHeaders(msg.getData().getAllHeaders())
	...
	if (sign) {
	    ...
	    boolean isRemoveCmsAlgorithmProtectionAttr = "true"
		    .equalsIgnoreCase(partnership.getAttribute(Partnership.PA_REMOVE_PROTECTION_ATTRIB));
	    dataBP = AS2Util.getCryptoHelper().sign(dataBP, senderCert, senderKey, digest, contentTxfrEncoding,
	...
	if ((t == null || t.length() < 1) && "true"
		.equalsIgnoreCase(partnership.getAttribute(Partnership.PA_SET_CONTENT_TRANSFER_ENCODING_OMBP))) {
	...
	if (customHeaders != null && customHeaders.length() > 0) {
		...
		if (logger.isTraceEnabled()) {
		    logger.trace("Adding custom header: " + headers[i] + " :::Split count:" + header.length
			    + msg.getLogMsgID());
		}
		if (header.length != 2) {
	...
	for (Map.Entry<String, String> entry : hdrs.entrySet()) {
	    ...
	    if (logger.isTraceEnabled()) {
		logger.trace("Added custom headers to outer MBP: " + entry.getKey() + "--->" + entry.getValue()
			+ msg.getLogMsgID());
	    }
	}
    }
	...
	try {
		...
		logger.trace("Pending info file written to:" + pendingInfoFile + "\n\tOriginal MIC: "
			...
			+ msg.getAttributes() + msg.getLogMsgID());
	    }
	...
	if (mdnOptions == null || mdnOptions.length() < 1) {
	    throw new OpenAS2Exception(
		    "Partner attribute " + Partnership.PA_AS2_MDN_OPTIONS + "is required but can be set to \"none\"");
	}
	...
	if (logger.isTraceEnabled()) {
	    ...
	    // different default
	    String tmic = AS2Util.getCryptoHelper().calculateMIC(mbp, dispOptions.getMicalg(), includeHeaders,
		    !msg.getPartnership().isPreventCanonicalization());
	    logger.trace("MIC outbound with forced reversed prevent canocalization: " + tmic + msg.getLogMsgID());
	    tmic = AS2Util.getCryptoHelper().calculateMIC(msg.getData(), dispOptions.getMicalg(), false,
		    msg.getPartnership().isPreventCanonicalization());
