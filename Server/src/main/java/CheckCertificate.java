...
public class CheckCertificate {
     ...
     * description
     */
    public String[][] opts = { { HOST, "server", "true", "true", "the target host name" },
	    { URI, "uri", "true", "false", "URI part of the connection" },
	    { PORT, "port", "true", "false", "target server port" },
	    { CACERT, "cacert", "true", "false", "Java keystore file to create if cert chain not present in Java keystore" },
	    { PASSWORD, "password", "true", "false", "password for Keystore if not 'changeit'" },
	    { DEBUG, "debug", "true", "false", "Enabling debug logging" },
	    { AUTH_USER, "authuser", "true", "false", "Basic auth user" },
	    { AUTH_PWD, "authpwd", "true", "false", "Basic auth password" },
	...
	for (String[] opt : opts) {
	    Option option = Option.builder(opt[0]).longOpt(opt[1]).hasArg("true".equalsIgnoreCase(opt[2])).desc(opt[4])
		    .build();
    ...
    public int CheckCertStore(String host, int port, String uri, String targetKeyStore, String keyStorePwd)
	    throws Exception {
    ...
    private void checkUsingApacheHttp(String host, int port, String uri, String targetKeyStore, String keyStorePwd) throws Exception {
	    ...
	    ResponseWrapper resp = HTTPUtil.execRequest(HTTPUtil.Method.POST,
		    "https://"+host+":"+port+"/"+uri, null, null, new ByteArrayInputStream("Testing".getBytes()),
		    httpOptions, 1000000000);
	...
	return dn.contains(" " + dnFld + "=\"") ? dn.replaceAll(".* " + dnFld + "=\"([^\"]*)\",[^$]*", "$1")
		: dn.replaceAll(".* " + dnFld + "=([^,]*),[^$]*", "$1");
    }
