...
public class OpenAS2ServerTest {
	...
	public static void startServers() throws Exception {
		...
		try {
			...
			serverA = new OpenAS2Server.Builder()
					.run(RESOURCE.get(partnerA.getName(), "config", "config.xml").getAbsolutePath());
			partnerA.setServer(serverA);

			serverB = new OpenAS2Server.Builder()
					.run(RESOURCE.get(partnerB.getName(), "config", "config.xml").getAbsolutePath());
	...
	private void verifyMessageDelivery(TestMessage testMessage) throws IOException {
		...
		File deliveredMsg = waitForFile(testMessage.toPartner.getInbox(), new PrefixFileFilter(testMessage.fileName),
				20, TimeUnit.SECONDS);
		...
		{
			File deliveryConfirmationMDN = waitForFile(testMessage.fromPartner.getRxdMDNs(),
					new PrefixFileFilter(testMessage.fileName), 10, TimeUnit.SECONDS);
			assertThat("Verify MDN was received by " + testMessage.fromPartner.getName(),
					deliveryConfirmationMDN.exists(), is(true));
		}

		{
			File deliveryConfirmationMDN = waitForFile(testMessage.toPartner.getSentMDNs(),
					new PrefixFileFilter(testMessage.fileName), 10, TimeUnit.SECONDS);
			assertThat("Verify MDN was stored by " + testMessage.toPartner.getName(),
					deliveryConfirmationMDN.exists(), is(true));
		}
	}
	...
	private static void enhancePartners() throws ComponentNotFoundException, FileNotFoundException {
		...
		partnerA.setSentMDNs(FileUtils.getFile(partnerA.getHome(), "data", partnershipFolderBtoA, "mdn",
				DateUtil.formatDate("yyyy-MM-dd")));
		partnerA.setRxdMDNs(FileUtils.getFile(partnerA.getHome(), "data", partnershipFolderAtoB, "mdn",
				DateUtil.formatDate("yyyy-MM-dd")));
		...
		partnerB.setSentMDNs(FileUtils.getFile(partnerB.getHome(), "data", partnershipFolderAtoB, "mdn",
				DateUtil.formatDate("yyyy-MM-dd")));
		partnerB.setRxdMDNs(FileUtils.getFile(partnerB.getHome(), "data", partnershipFolderBtoA, "mdn",
				DateUtil.formatDate("yyyy-MM-dd")));
