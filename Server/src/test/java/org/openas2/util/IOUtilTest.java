...
public class IOUtilTest {
	...
	public Record(String filename, String format, String delimiters, boolean mergeExtraTokens,
		String expectFileName, String expectFrom, String expectTo) {
    ...
    {
	add(new Record("MyCo-PartnerCo-MyFileNameWithExtension.edi","sender.as2_id, receiver.as2_id,"
		+ " attributes.filename", "-.",false,"MyFileNameWithExtension","MyCo","PartnerCo"));
	add(new Record("MyCo-PartnerCo-MyFileNameWithExtension.edi","sender.as2_id, receiver.as2_id,"
		+ " attributes.filename","-.",true,"MyFileNameWithExtension.edi","MyCo","PartnerCo"));
    }};
     ...
     */
    private String[][] testFilePollingFilters = {
	    { "File1.txt", "txt", "", "1" },
	    { "File2.edi", "txt, edi", "", "1" },
	    { "File3.tmp", "", "temp", "1" },
	    // Tests that should not return a file
	    { "File4.txt", "edi", "", "0"},
	    { "File5.edi", "txt, edint", "", "0" },
	    { "File6.tmp", "", "tmp", "0" },
	    { "File7.tmp", "", "part , tmp", "0" }
    };
	...
	for (Record record : testParameterParsingData) {
	    ...
	    assertThat(
		    "Check " + expectFilename + " against "
			    + params.getParameter(MessageParameters.KEY_ATTRIBUTES + ".filename"),
		    params.getParameter(MessageParameters.KEY_ATTRIBUTES + ".filename")
			    .matches(Pattern.quote(expectFilename)),
		    is(true));
	    assertThat(
		    "Check receiver AS2 ID " + record.expectTo + " against "
			    + params.getParameter(MessageParameters.KEY_RECEIVER + ".as2_id"),
		    params.getParameter(MessageParameters.KEY_RECEIVER + ".as2_id")
			    .matches(Pattern.quote(record.expectTo)),
		    is(true));
	    assertThat(
		    "Check sender AS2 ID " + record.expectFrom + " against "
			    + params.getParameter(MessageParameters.KEY_SENDER + ".as2_id"),
		    params.getParameter(MessageParameters.KEY_SENDER + ".as2_id")
			    .matches(Pattern.quote(record.expectFrom)),
		    is(true));
	}
    }
	...
	for (String[] strings : testFilePollingFilters) {
	    ...
	    assertThat( "Check that a file is detected for " + name.getName() + ":" + allow + ":" + exclude,
		    files.length == Integer.parseInt(strings[3]),
		    is(true));
